...
use crate::arch::riscv::mm::{VirtAddr, PhysAddr, paging};
        ...
        for i in 0..DEVICES_AVAILABLE.len() {
            match &DEVICES_AVAILABLE[i] {
                Device::GEM(gem) => {
                    ...
                    debug!("Init GEM at {:x}, irq: {}, phy_addr: {}", gem.base, gem.irq, gem.phy_addr);
                    paging::identity_map::<paging::HugePageSize>(PhysAddr(gem.base as u64), PhysAddr((gem.base + gem.size - 1) as u64));
                    match gem::init_device(VirtAddr(gem.base as u64), gem.irq.into(), gem.phy_addr.into()) {
                ...
                Device::PLIC(plic) => {
                    ...
                    paging::identity_map::<paging::HugePageSize>(PhysAddr(plic.base as u64), PhysAddr((plic.base + plic.size -1) as u64));
                    init_plic(plic.base);
                },
            }
        }
...
fn walk_nodes<'a, 'b>(dtb: &Dtb<'a>, path: &'b str, level: usize) {
    ...
    for prop in dtb.enum_properties(path) {
        debug!("{}Prop: {}: {:x?}", "\t".repeat(level), prop, dtb.get_property(path, prop));
    }
    for node in dtb.enum_subnodes(path) {
        ...
        if node.starts_with("ethernet@") {
            ...
            let compatible = core::str::from_utf8(
                dtb.get_property(path, "compatible").expect("compatible property for ethernet not found in dtb")
            ).unwrap();
            debug!("Compatible: {}", compatible);

            if compatible.contains("sifive,fu540-c000-gem"){
                let reg = dtb.get_property(path, "reg").expect("Reg property for ethernet not found in dtb");
                ...
                let interrupts = dtb.get_property(path, "interrupts").expect("Reg property for ethernet not found in dtb");
                let irq: u8 = interrupts[3];

                let path = &[path, "ethernet-phy"].concat();
                debug!("{}", path);
                let phy = dtb.get_property(path, "reg").expect("Reg property for ethernet-phy not found in dtb");
            ...
            let compatible = core::str::from_utf8(
                dtb.get_property(path, "compatible").expect("compatible property for interrupt-controller not found in dtb")
            ).unwrap();
            debug!("Compatible: {}", compatible);
            if compatible.contains("sifive,plic-1.0.0"){
                let reg = dtb.get_property(path, "reg").expect("Reg property for plic not found in dtb");
                ...
                unsafe {
                    //Insert before
                    DEVICES_AVAILABLE.insert(0, Device::PLIC(Plic {
