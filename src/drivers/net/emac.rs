...
// CCU
const CCU_BASE: usize = 0x02001000;
const EMAC_BGR_REG: *mut u32 = (0x97C + CCU_BASE) as *mut u32;

// SYS_CFG
const SYS_CFG_BASE: usize = 0x03000000;
const EMAC_EPHY_CLK_REG0: *mut u32 = (0x30 + SYS_CFG_BASE) as *mut u32;

// GPIO
const GPIO_BASE: usize = 0x02000000;
const PE_CFG0: *mut u32 = (0xc0 + GPIO_BASE) as *mut u32;
const PE_CFG1: *mut u32 = (0xc4 + GPIO_BASE) as *mut u32;
const PE_CFG2: *mut u32 = (0xc8 + GPIO_BASE) as *mut u32;
const PE_DAT: *mut u32 = (0xd0 + GPIO_BASE) as *mut u32;

register_structs! {
	/// Register offsets
	Registers {
		// Control register: read-write
		(0x000 => basic_ctl0: ReadWrite<u32, BasicCtl0::Register>),
		(0x004 => basic_ctl1: ReadWrite<u32, BasicCtl1::Register>),
		(0x00C => int_en: ReadWrite<u32, IntEn::Register>),
		(0x008 => _reserved1),
		(0x014 => tx_ctl1: ReadWrite<u32, TXCtl1::Register>),
		(0x018 => _reserved2),
		(0x020 => tx_dma_list: ReadWrite<u32>),
		(0x024 => _reserved3),
		(0x028 => rx_ctl1: ReadWrite<u32, RXCtl1::Register>),
		(0x02C => _reserved4),
		(0x034 => rx_dma_list: ReadWrite<u32>),
		(0x038 => _reserved5),
		(0x048 => mii_cmd: ReadWrite<u32, MIICmd::Register>),
		(0x04C => mii_data: ReadWrite<u32>),
...
register_bitfields! [
	...
	BasicCtl1 [
		RX_TX_PRI	OFFSET(1) NUMBITS(1) [],
		SOFT_RST	OFFSET(0) NUMBITS(1) [],
	],
	TXCtl1 [
		TX_DMA_START	OFFSET(31) NUMBITS(1) [],
		TX_DMA_EN		OFFSET(30) NUMBITS(1) [],
		TX_TH			OFFSET(8) NUMBITS(3) [],
		TX_MD			OFFSET(1) NUMBITS(1) [],
		FLUSH_TX_FIFO	OFFSET(0) NUMBITS(1) [],
	],
	RXCtl1 [
		RX_DMA_START			OFFSET(31) NUMBITS(1) [],
		RX_DMA_EN				OFFSET(30) NUMBITS(1) [],
		RX_FIFO_FLOW_CTL		OFFSET(24) NUMBITS(1) [],
		RX_FLOW_CTL_TH_DEACT	OFFSET(22) NUMBITS(2) [],
		RX_FLOW_CTL_TH_ACT		OFFSET(20) NUMBITS(2) [],
		RX_TH					OFFSET(4) NUMBITS(2) [],
		RX_ERR_FRM				OFFSET(3) NUMBITS(1) [],
		RX_RUNT_FRM				OFFSET(2) NUMBITS(1) [],
		RX_MD					OFFSET(1) NUMBITS(1) [],
		FLUSH_RX_FRM			OFFSET(0) NUMBITS(1) [],
	],

	IntEn [
		RX_EARLY_INT_EN			OFFSET(13) NUMBITS(1) [],
		EX_OVERFLOW_INT_EN		OFFSET(12) NUMBITS(1) [],
		RX_TIMEOUT_INT_EN		OFFSET(11) NUMBITS(1) [],
		RX_DMA_STOPPED_INT_EN	OFFSET(10) NUMBITS(1) [],
		RX_BUF_UA_INT_EN		OFFSET(9) NUMBITS(1) [],
		RX_INT_EN				OFFSET(8) NUMBITS(1) [],
		TX_EARLY_INT_EN			OFFSET(5) NUMBITS(1) [],
		TX_UNDERFLOW_INT_EN		OFFSET(4) NUMBITS(1) [],
		TX_TIMEOUT_INT_EN		OFFSET(3) NUMBITS(1) [],
		TX_BUF_UA_INT_EN		OFFSET(2) NUMBITS(1) [],
		TX_DMA_STOPPED_INT_EN	OFFSET(1) NUMBITS(1) [],
		TX_INT_EN				OFFSET(0) NUMBITS(1) [],
	],
	MIICmd [
		MDC_DIV_RATIO_M	OFFSET(20) NUMBITS(3) [
			CLK_DIV16 = 0b000,
			CLK_DIV32 = 0b001,
			CLK_DIV64 = 0b010,
			CLK_DIV128 = 0b011,
		],
		PHY_ADDR		OFFSET(12) NUMBITS(5) [],
		PHY_REG_ADDR	OFFSET(4) NUMBITS(5) [],
		MII_WR			OFFSET(1) NUMBITS(1) [
			READ = 0b0,
			WRITE = 0b1,
		],
...
impl NetworkInterface for EMACDriver {
...
pub fn init_device(
	...
	phy_addr: u32,
	mac: [u8; 6],
) -> Result<EMACDriver, DriverError> {
	debug!("Init EMAC at {:p}", base);

	let emac = base.as_mut_ptr::<Registers>();

	
	unsafe {
		// Reset
		core::ptr::write_volatile(EMAC_BGR_REG, 0);
		// Deassert
		core::ptr::write_volatile(EMAC_BGR_REG, 1 << 16);
		// Enable
		core::ptr::write_volatile(EMAC_BGR_REG, 1 << 0 |1 << 16);

		// GPIO
		core::ptr::write_volatile(PE_CFG0, 0x88888888);
		core::ptr::write_volatile(PE_CFG1, 0x88);

		//Reset
		core::ptr::write_volatile(PE_CFG2, 0x01);
		core::ptr::write_volatile(PE_DAT, 0 << 16);
		crate::arch::kernel::processor::udelay(50000);
		core::ptr::write_volatile(PE_DAT, 1 << 16);
		crate::arch::kernel::processor::udelay(50000);
		// Drive strength?

		// Configure clock
		core::ptr::write_volatile(EMAC_EPHY_CLK_REG0, 0b00_1_000_00000_0_0_0_00);

		// Initialize the controller
		// Soft reset
		(*emac).basic_ctl1.modify(BasicCtl1::SOFT_RST::SET);
		while (*emac).basic_ctl1.is_set(BasicCtl1::SOFT_RST) {}
		debug!{"EMAC soft reset success"};

		// Set DMA priority
		(*emac).basic_ctl1.modify(BasicCtl1::RX_TX_PRI::SET);
		...
		(*emac).int_en.modify(IntEn::RX_INT_EN::SET);

		// Configure TX DMA list
		(*emac).tx_dma_list.set(0);
		// Configure RX DMA list
		(*emac).rx_dma_list.set(0);
		...
		let phy_status = phy_read(emac, phy_addr, PhyReg::Status);

		debug!{"phy status: {:?}", phy_status};
		...
		//return Err(DriverError::InitGEMDevFail(GEMError::NoPhyFound));
			...
			// Enable AN
			let phy_control = phy_read(emac, phy_addr, PhyReg::Control);
			phy_write(
				emac,
				phy_addr,
				PhyReg::Control,
			...
			while (phy_read(emac, phy_addr, PhyReg::Status) | PhyStatus::ANCompleteMask as u16) == 0
			{
			}

			// Read partner ability register
			let partner_ability = phy_read(emac, phy_addr, PhyReg::ANLinkPartnerAbility);
	...
	// 	// Configure AHB fixed burst length to INCR16
	...
	// 	return Err(DriverError::InitEMACDevFail(EMACError::Unknown));
	// }
...
unsafe fn phy_read(emac: *mut Registers, addr: u32, reg: PhyReg) -> u16 {
	// Check that no MDIO operation is in progress
	wait_for_mdio(emac);
	// Initiate the data shift operation over MDIO
	(*emac).mii_cmd.modify(
		MIICmd::MDC_DIV_RATIO_M::CLK_DIV128
			+ MIICmd::MII_WR::READ
			+ MIICmd::PHY_ADDR.val(addr)
			+ MIICmd::PHY_REG_ADDR.val(reg as u32)
			+ MIICmd::MII_BUSY::SET
	);
	debug!("Wait read");
	wait_for_mdio(emac);
	(*emac).mii_data.get() as u16
}

unsafe fn phy_write(emac: *mut Registers, addr: u32, reg: PhyReg, data: u16) {
	// Check that no MDIO operation is in progress
	wait_for_mdio(emac);
	// Initiate the data shift operation over MDIO
	(*emac).mii_data.set(data as u32);
...
unsafe fn wait_for_mdio(emac: *mut Registers) {
	// Check that no MDIO operation is in progress
	debug!{"Wait"};
	while (*emac).mii_cmd.is_set(MIICmd::MII_BUSY) {}
	debug!{"Wait end"};
}
