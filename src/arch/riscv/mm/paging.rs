...
impl PageTableEntry {
	...
	fn set(&mut self, physical_address: PhysAddr, flags: PageTableEntryFlags, is_leaf: bool) {
...
impl<L: PageTableLevel> PageTableMethods for PageTable<L> {
	...
	fn map_page_in_this_table<S: PageSize>(
		...
		trace!("Address: {:p}, index: {}", &self, index);
		let flush = self.entries[index].is_present();

		self.entries[index].set(physical_address, S::MAP_EXTRA_FLAG | flags, true);
	...
	fn map_page<S: PageSize>(
		...
		if L::LEVEL > S::MAP_LEVEL {
			...
			if !self.entries[index].is_present() {
				...
				self.entries[index].set(new_entry, PageTableEntryFlags::BLANK, false);

				debug!("new_entry {:#X}", new_entry);
				debug!("new_entry {:?}", self.entries[index]);

				unsafe {
					debug!("root {:p}", &ROOT_PAGETABLE);
					debug!("new_entry addr {:p}", &self.entries[index]);
				}
...
pub fn init_page_tables() {
	...
	unsafe {
		...
		trace!("Before: {:x}", satp::read().bits());
		satp::write(0x8 << 60 | ((&ROOT_PAGETABLE as *const _ as usize) >> 12));
		assert!((&ROOT_PAGETABLE as *const _ as usize & 0xfff) == 0);
		sfence_vma(0, 0);
	}
	trace!("After: {:x}", satp::read().bits());
